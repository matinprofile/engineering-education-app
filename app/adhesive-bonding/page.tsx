      ...
      <section className="relative overflow-hidden border-b border-[color:var(--border)] bg-primary/55">
        ...
        <div className="relative mx-auto w-full max-w-7xl px-4 py-20 sm:px-6 sm:py-24 lg:px-8">
          ...
          <div className="flex flex-col gap-6 md:flex-row md:items-start md:justify-between">
            <div>
              <h1 className="font-heading text-4xl font-bold leading-tight text-text sm:text-5xl lg:text-6xl">Adhesive Bonding</h1>
              <p className="mt-4 text-lg font-medium text-accent/80">Interactive process, design, and failure simulations</p>
            </div>
          ...
          <p className="mt-5 max-w-3xl text-base leading-8 text-muted sm:text-lg">
            Access adhesive bonding simulations for surface preparation, joint manufacturing, failure analysis, and design guidance.
          </p>
        </div>
      </section>

      <SectionWrapper title="Interactive Tools" subtitle="Open a simulation to explore a specific adhesive bonding topic.">
        <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
          {adhesiveBondingTools.map((tool) => (
            <Link
              key={tool.slug}
